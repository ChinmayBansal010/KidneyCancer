...
def extract_case(case_dir: Path, out_root: str | Path = OUT_ROOT):
...
def extract_all(
    tumor_roi_root: str | Path | None = TUMOR_ROI_ROOT,
    out_root: str | Path | None = OUT_ROOT,
):
    root = Path(tumor_roi_root or TUMOR_ROI_ROOT)
    out_root = Path(out_root or OUT_ROOT)
    if not root.exists():
        raise FileNotFoundError(f"Tumor ROI root does not exist: {root}")
    cases = sorted(root.glob("case_*"))
    if not cases:
        raise FileNotFoundError(f"No tumor ROI case directories found in: {root}")

    for case in tqdm(cases, desc="Patch extraction"):
        extract_case(case, out_root=out_root)

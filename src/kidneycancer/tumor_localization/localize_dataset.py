...
def localize_all(
    kidney_roi_root: str | Path | None = KIDNEY_ROI_ROOT,
    out_root: str | Path | None = OUT_ROOT,
):
    root = Path(kidney_roi_root or KIDNEY_ROI_ROOT)
    out_root = Path(out_root or OUT_ROOT)
    if not root.exists():
        raise FileNotFoundError(f"Kidney ROI root does not exist: {root}")
    cases = sorted(root.glob("case_*"))
    if not cases:
        raise FileNotFoundError(f"No kidney ROI case directories found in: {root}")
